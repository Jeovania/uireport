    ...
    respond_to do |format|
      if @profile_answer.save
        ...
        if @perguntaid < @p_total
          format.html { redirect_to new_profile_project_answer_path(:project_id => @project.id, :p => @perguntaid+1) }
        else
          format.html { redirect_to profile_para_avaliar_path, notice: 'Avaliação completa. Obrigado! :)' }    
        end  
        format.json { render :show, status: :created, location: @profile_answer }
      else
        format.html { redirect_to new_profile_project_answer_path(:project_id => @project.id, :p => @perguntaid), alert: 'Erro. Projeto já avaliado.'}
